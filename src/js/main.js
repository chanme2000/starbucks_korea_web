...
// 콜백 함수로 lodash의 throttle을 사용하여 
// 0.3초마다 실행한 결과를 반환하도록 처리
// _.throttle(콜백함수, 시간);
window.addEventListener('scroll', _.throttle(function(){
  ...
  if(window.scrollY > 500) {
    ...
    // 스크롤 값이 500이상이 되면 단순하게 보이게 처리하는 것이 아닌
    // 부드러운 효과를 적용하고자 한다.
    // gsap.to(요소, 지속시간, {옵션})
    gsap.to(badgeEl, .6, {
      opacity : 0,
...
// 이미지 영역을 fade--in으로 그룹처리 해준 이유가 된다.
